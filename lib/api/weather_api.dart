...
class WeatherApi {
  Future<WeatherModelClass> getWeather(String query) async {
    //쿼리를 통해 쿼리 검색한거에 해당하는 정보 다가져옴
    Uri url = Uri.parse(
    ...
    String weather =
        json['weather'][0]['main'] as String; //웨더 스트링에서 첫번째 덩어리에서 메인의 밸류값
    //여기서 필요한걸 가져오고
    num wind = json['wind']['speed'] as num;
    print(wind);
    
    return WeatherModelClass(
        name: weatherName, temp: weatherTemp, weather: weather, wind: wind);
  }
}
